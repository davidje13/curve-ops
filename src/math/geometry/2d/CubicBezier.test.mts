...
describe('bezier3LengthEstimate', () => {
	...
	it('works with edge cases', () => {
		const PT10 = { x: 1, y: 0 };
		checkCurveLength(bezier3FromPts(PT00, PT00, PT00, PT00), 0);
		checkCurveLength(bezier3FromPts(PT00, PT00, PT00, PT10), 1);
		checkCurveLength(bezier3FromPts(PT00, PT00, PT10, PT10), 1);
		checkCurveLength(bezier3FromPts(PT00, PT10, PT10, PT10), 1);
		checkCurveLength(bezier3FromPts(PT00, PT10, PT00, PT10), 1);
		checkCurveLength(
			bezier3FromPts(PT00, PT10, { x: 2, y: 0 }, { x: 3, y: 0 }),
			3,
		);
	});
	...
	it('is accurate near edge cases', () => {
		...
		for (let i = 1; i < 100; ++i) {
			...
			checkCurveLength(bezier3FromPts(PT00, PT00, { x: d, y: 0 }, PT10), 1);
			checkCurveLength(bezier3FromPts(PT00, { x: 1 - d, y: 0 }, PT10, PT10), 1);
			checkCurveLength(bezier3FromPts(PT00, PT00, PT00, { x: d, y: 0 }), d);
			checkCurveLength(
				bezier3FromPts(PT00, PT00, { x: d, y: 0 }, { x: d, y: 0 }),
				d,
			);
			checkCurveLength(
				bezier3FromPts(
