	...
	begin = (pt: Pt) => {
		...
		return {
			move: (pt: Pt, done = false) => {
				...
				if (fit.d < this._minError || best.n < 2 || fit.d < best.d) {
					best = fit;
				}
				if (fit.d > this._maxError || live.length > this.maxN) {
					this.onSegment(best.c, best.n);
...
function doFit(points: readonly PtWithDist[], prevControl: Pt | null): Fit {
	if (!points.length) {
